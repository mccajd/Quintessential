...
func transform(type: String, item_keys):
	match type:
		...
		"sublimation":
			var rnd = _transform(Items.sublimation_recipes, item_keys, ["salt"])
			return rnd if rnd != null else _transform(Items.get_double_sublimation_recipes(), item_keys, ["salt", "salt"])
		"distillation":
